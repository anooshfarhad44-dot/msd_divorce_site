...
export default function HeroSection() {
  return (
    <section className="relative bg-slate-950 text-white overflow-hidden min-h-[90vh] flex flex-col">
      {/* 
        GUARANTEED INLINE CSS ANIMATIONS 
        Yeh inline styles humare dynamic classes ko direct browser-level control dete hain bina globals.css par depend kiye.
      */}
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @keyframes neonPulse {
          ...
          50% { box-shadow: 0 0 30px rgba(245, 158, 11, 0.75), 0 0 60px rgba(245, 158, 11, 0.35); }
        }
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center w-full">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-5 mb-12 items-start sm:items-center">
              ...
              {/* BUTTON 1: Speak to Solicitor (Dynamic Fluid, Real-Time Gradient & Float) */}
              <Link href={heroData.cta1.href}
                className="relative inline-flex items-center justify-center gap-2.5 text-white font-bold px-8 py-4 rounded-2xl text-base overflow-hidden group active:scale-95 transition-all duration-300
                  animate-float-btn animate-neon-pulse animate-fluid-gradient bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:brightness-110">
                
                {/* Continuous Shimmer Light Sweep */}
                <span className="absolute inset-0 w-[40%] h-full bg-gradient-to-r from-transparent via-white/25 to-transparent animate-shimmer-sweep pointer-events-none" />
                
                {/* Phone icon */}
                <svg className="w-5 h-5 relative z-10 transition-transform duration-300 group-hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                
                <span className="relative z-10 tracking-wide">{heroData.cta1.label}</span>
              </Link>

              {/* BUTTON 2: Read Guide (Subtle Floating & Accent Transitions) */}
              <Link href="/guide"
                className="relative inline-flex items-center justify-center gap-2 border border-white/10 bg-slate-900/40 text-slate-200 font-semibold px-8 py-4 rounded-2xl text-base group overflow-hidden
                  animate-subtle-bounce active:scale-95 transition-all duration-300 backdrop-blur-md
                  hover:border-amber-500/40 hover:text-white hover:shadow-[0_0_20px_rgba(245,158,11,0.12)]">
                
                {/* Slow pulse glow behind the text inside */}
                <span className="absolute inset-0 bg-gradient-to-r from-amber-500/0 via-amber-500/5 to-amber-500/0 opacity-40 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
                
                {/* Book Icon */}
                <svg className="w-5 h-5 text-slate-400 group-hover:text-amber-400 transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                </svg>
                
                <span className="relative z-10 transition-colors duration-300">Read the Divorce Guide</span>
                
                {/* Arrow slide */}
                <svg className="w-4 h-4 text-slate-500 group-hover:text-amber-400 transition-all duration-300 transform group-hover:translate-x-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                </svg>
              </Link>
            </div>
